...
export class AuthService {
	...
	async login42(token: string, refresh_token: string){
		...
		try{
			...
			const newUser = await this.userService.createUser({token, refresh_token, mail: email, Uid: login, username: login, level: 0, MMR: 0, pic: ImgData, desc: 'new user', history: '', nbGamePlayed: 0, victory: 0, is42: true, password: "none", socketId: 'none', status: -1});
			return newUser;
		} catch (error) {
	...
	async register(data: any) {
		...
		try {
			...
			const ImgData: Buffer = Buffer.alloc(0);
			const newUser = await this.userService.createUser({token : 'none', refresh_token: 'none', mail: data.email, Uid:data.username, username: data.username, level: 0, MMR: 0, pic: ImgData, desc: 'new user', history: '', nbGamePlayed: 0, victory: 0, is42: false, password: hashPassword, socketId: 'none', status: -1});
			return newUser;
		} catch (error) {
